...
export class FacebookConnector implements OAuthConnector {
  ...
  private get clientId(): string {
    ...
    if (!id) throw new InternalServerErrorException("FACEBOOK_CLIENT_ID is not set");
    return id;
  }

  private get clientSecret(): string {
    const secret = process.env["FACEBOOK_CLIENT_SECRET"];
    if (!secret) throw new InternalServerErrorException("FACEBOOK_CLIENT_SECRET is not set");
    return secret;
  }
